            ...
            public void onSuccess(File rawInp) {
                ...
                executor.execute(() -> {
                    try {
                        ...
                        String dirLabel = (loadDof == 1) ? "Horizontal X (DOF 1)" : (loadDof == 3) ? "Lateral Depth Z (DOF 3)" : "Vertical Y (DOF 2)";
                        logger.info("FEA 3D Solved: Element=" + elemType + " | Load=" + finalLoadMagnitude + " N (" + dirLabel + ")");
                        cleanIntermediateSimulationFiles(workDir);
