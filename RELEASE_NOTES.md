...
* **Convergencia Asintótica y Malla Multinivel (7 Niveles):**
  * Verificación y calibración de 7 niveles de refinamiento métrico con Gmsh:
    ...
    * **Diferenciación Dinámica y Transparente en la Interfaz (UI):**
      * **Indicador en Vivo de Perfil de Hardware:** Debajo del selector de malla se muestra la RAM física real y número de núcleos (`📱 Perfil Estándar`, `⚡ Perfil Flagship 12GB` o `🔥 Máxima Potencia Extrema`). Así el usuario sabe exactamente en qué variante de hardware está calculando y por qué su equipo rinde a cierta velocidad.
      * Al tocar o intentar deslizar al Nivel 7 en hardware no compatible, el slider se tranca en el Nivel 6 con un mensaje explicativo y respetando la libertad total del Nivel 6.
