...
echo "Verificando init.tcl (requerido por install(FILES ...) de OpenSees)..."
INIT_TCL=$(find "$FAKE_USR/lib" -maxdepth 2 -name 'init.tcl' 2>/dev/null | head -n1)
if [ -z "$INIT_TCL" ]; then
    echo "Aviso: init.tcl no encontrado, copiando manualmente desde el código fuente..."
    mkdir -p "$FAKE_USR/lib/tcl8.6"
    cp -r "$HOME/tcl/library/"* "$FAKE_USR/lib/tcl8.6/"
else
    echo "OK: init.tcl encontrado en $INIT_TCL"
fi
